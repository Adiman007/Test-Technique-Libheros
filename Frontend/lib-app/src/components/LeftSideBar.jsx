...
const LeftSidebar = ({ isVisible, userId }) => {
  const [todoLists, setTodoLists] = useState([]); 
  const [newTodoName, setNewTodoName] = useState(''); 
  const [isLoading, setIsLoading] = useState(false); 
  const [selectedList, setSelectedList] = useState(null); // Track the selected list
  ...
  const handleAddTodoList = async () => {
    if (newTodoName.trim() === ''){ alert("Todolist name can't be blank"); return;}
